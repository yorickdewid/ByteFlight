        ...
        <div className="w-48 border-r border-slate-800 p-3 flex flex-col gap-0.5">
          ...
          {tabs.map(t => (
            <button
              ...
              className={`flex items-center gap-2.5 px-3 py-2 rounded-md text-sm transition-colors ${activeTab === t.id ? 'bg-slate-800 text-slate-100' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
            >
              <t.icon size={15} />
              {t.label}
            </button>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="border border-sky-500 bg-sky-500/10 p-3 rounded-xl flex items-center gap-2.5 cursor-pointer">
                      <Moon size={15} className="text-sky-400" />
                      <div className="text-sm text-white">Dark</div>
                    </div>
                    <div className="border border-slate-700/50 bg-slate-800/40 p-3 rounded-xl flex items-center gap-2.5 opacity-50 cursor-not-allowed">
                ...
                <div className="pt-4 border-t border-slate-800">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`p-3 rounded-xl border flex items-center justify-between transition-colors ${fuelPolicy === 'VFR_DAY' ? 'bg-sky-500/10 border-sky-500 text-white' : 'bg-slate-800/40 border-slate-700/50 text-slate-400 hover:border-slate-700'}`}
                    >
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl border flex items-center justify-between transition-colors ${fuelPolicy === 'VFR_NIGHT' ? 'bg-sky-500/10 border-sky-500 text-white' : 'bg-slate-800/40 border-slate-700/50 text-slate-400 hover:border-slate-700'}`}
                    >
