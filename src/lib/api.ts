...
export const ApiService = {
  ...
  buildRouteString(departure: NavPoint, waypoints: Waypoint[], arrival: NavPoint): string {
    // Valid aviation identifiers: 2-5 uppercase letters (ICAO, VOR, NDB, FIX)
    // Excludes alphanumeric mixes like wp1, wp2 (legacy localStorage IDs)
    const isNavIdentifier = (id: string) => /^[A-Z]{2,5}$/.test(id.toUpperCase());
